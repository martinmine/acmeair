...
cf ic run -e CCS_BIND_APP=${MONGO_BRIDGE} -e SERVICE_NAME=auth     -e SD_URL=${SD_URL} -e SD_TOKEN=${SD_TOKEN} --name auth_java_1     ${REGISTRY}/${NAME_SPACE}/acmeair_java_authservice
cf ic run -e CCS_BIND_APP=${MONGO_BRIDGE} -e SERVICE_NAME=booking  -e SD_URL=${SD_URL} -e SD_TOKEN=${SD_TOKEN} --name booking_java_1  ${REGISTRY}/${NAME_SPACE}/acmeair_java_bookingservice
cf ic run -e CCS_BIND_APP=${MONGO_BRIDGE} -e SERVICE_NAME=customer -e SD_URL=${SD_URL} -e SD_TOKEN=${SD_TOKEN} --name customer_java_1 ${REGISTRY}/${NAME_SPACE}/acmeair_java_customerservice
cf ic run -e CCS_BIND_APP=${MONGO_BRIDGE} -e SERVICE_NAME=flight   -e SD_URL=${SD_URL} -e SD_TOKEN=${SD_TOKEN} --name flight_java_1   ${REGISTRY}/${NAME_SPACE}/acmeair_java_flightservice
